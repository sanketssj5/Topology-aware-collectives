...
void ringAlgo(int myRank, int size, int D, MPI_Comm comm)
{
    int  **arr;
    ...
    for (int step = 1; step < size; step++)
    {
        ...
        MPI_Isend(arr[index], D, MPI_INT, (myRank + 1) % size, myRank, comm, &req1);
        MPI_Irecv(arr[++index], D, MPI_INT, MPI_ANY_SOURCE, MPI_ANY_TAG, comm, &req2);
...
int main(int argc, char **argv)
{
    ...
    ringAlgo(row_rank, size, D, row_comm);
